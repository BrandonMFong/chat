...
BFTEST_UNIT_FUNC(test_AsymCheckReady, 2<<2, {
...
BFTEST_UNIT_FUNC(test_AsymSimpleString, 2<<2, {
	Cipher * c = Cipher::create(kCipherTypeAsymmetric);
	if (c == 0) {
		result = 1;
	} else if (c->genkey()) {
...
BFTEST_UNIT_FUNC(test_AsymLongString, 2<<2, {
	Cipher * c = Cipher::create(kCipherTypeAsymmetric);
	if (c == 0) {
		result = 1;
	} else if (c->genkey()) {
...
BFTEST_UNIT_FUNC(test_HandingOffPublicKeyToEncrypt, 2<<2, {
	Data pub;
...
BFTEST_COVERAGE_FUNC(cipherasymmetric_tests, {
	BFTEST_LAUNCH(test_AsymSimpleString);
	BFTEST_LAUNCH(test_AsymLongString);
	BFTEST_LAUNCH(test_HandingOffPublicKeyToEncrypt);
	BFTEST_LAUNCH(test_AsymCheckReady);
})
